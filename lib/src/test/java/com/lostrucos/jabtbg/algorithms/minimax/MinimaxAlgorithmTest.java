...
public class MinimaxAlgorithmTest {
    ...
    private static MinimaxAlgorithm minimaxAlgorithm;

    @BeforeAll
    public static void setUp() {
        minimaxAlgorithm = new MinimaxAlgorithm();
        minimaxAlgorithm.initialize(game, player);
    }
    ...
    public void testChooseAction() {
        ...
        when(game.getPlayerActions(eq(player.getPlayerIndex()), any(GameState.class))).thenReturn((List) possibleActions);
        ...
        when(nextGameState.getUtility(player.getPlayerIndex())).thenReturn(10.0, -10.0); // First call returns 10, second returns -10

        // Execute
        Action chosenAction = minimaxAlgorithm.chooseAction(initialGameState);

        // Verify
        assertEquals(action1, chosenAction);

        verify(game).getPlayerActions(player.getPlayerIndex(), initialGameState);
        ...
        verify(nextGameState, times(2)).getUtility(player.getPlayerIndex());
    }
    ...
    public void testMaxValue() {
        ...
        when(initialGameState.getUtility(player.getPlayerIndex())).thenReturn(42.0);

        // Execute
        double value = minimaxAlgorithm.minMaxValue(initialGameState, true);

        // Verify
        assertEquals(42, value, 0.01);

        verify(initialGameState).isTerminalNode();
        verify(initialGameState).getUtility(player.getPlayerIndex());
    }
    ...
    public void testMinValue() {
        ...
        // Execute
        double value = minimaxAlgorithm.minMaxValue(initialGameState, false);
